...
# 新增：Noto Sans CJK .ttc 字体索引映射（固定值）
TTC_INDEX_MAP = {
    'zh-Hans': 0,  # 简体中文
    'zh-Hant': 1,  # 繁体中文
    'ja':      2,  # 日文
...
# 修改：适配 .ttc 字体的字符检查（指定索引）
def check_font_has_text(font_path, text, font_index=0):
    ...
    try:
        ...
        logging.debug(f"检查字体 {font_path}（索引 {font_index}）失败: {str(e)}")
        return False

# 修改：适配 .ttc 字体的诊断（指定索引）
def diagnose_font_issues(text, font_path, font_index=0):
    ...
    try:
        ...
        if missing:
            logging.warning(f"检测到{len(missing)}个无法渲染的字符（字体：{os.path.basename(font_path)}，索引：{font_index}）。")
...
def list_existing_paths(paths):
    ...
    logging.debug(f"存在的字体路径：{existing}")
    return existing
...
def try_load_font(path, size, index=0):
    try:
        ...
        logging.debug(f"加载字体失败：{path}（索引 {index}）：{str(e)}")
        return None
...
# 重点修改：适配 .ttc 字体索引，优先使用已安装的路径
def get_font_for_language(font_size, lang, sample_text=""):
    candidates = []
    # 优先使用已确认存在的 Noto CJK 路径（从 fc-list 输出确认）
    ...
    if lang in {'zh-Hans', 'zh-Hant', 'ja', 'ko'}:
        candidates += [noto_cjk_regular, noto_cjk_bold]
        # 保留原后备路径
        candidates += [
    ...
    # 尝试路径加载（针对 CJK 语言指定索引）
    for path in list_existing_paths(candidates):
        # 获取当前语言对应的 .ttc 索引（非 CJK 用 0）
        font_index = TTC_INDEX_MAP.get(lang, 0)
        # 加载字体
        f = try_load_font(path, font_size, index=font_index)
        if f:
            # 检查字体是否支持样本文本（传入索引）
            if not sample_text or check_font_has_text(path, sample_text, font_index=font_index):
                logging.info(f"成功选择字体：{os.path.basename(path)}（语言：{lang}，索引：{font_index}）")
                return f, path
...
if __name__ == "__main__":
    ...
    if not success:
        ...
        logging.error("推荐安装: sudo apt install fonts-noto-cjk fonts-noto-cjk-extra fonts-noto -y")
        logging.error("=" * 50)
    else:
        logging.info("\n" + "=" * 50)
