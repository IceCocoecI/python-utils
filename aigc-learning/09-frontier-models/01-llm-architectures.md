...
> 目标：搞懂主流大语言模型的架构设计——从 Transformer 原论文到 Qwen3、DeepSeek-V3/R1 等 2025 前沿模型。
...
**结论**：Decoder-only 在大规模生成式 LLM 中最容易扩展、工程链路最成熟，因此成为当前主流。
...
| GPT-4o | 2024 | 未公开 | 低延迟多模态交互，文本/视觉/语音体验统一 |

> GPT-4 / GPT-4o 的具体参数量、专家数和训练数据未公开。学习时可以分析它们的能力边界、接口、延迟和产品体验，
> 但不要把社区传闻当成确定架构。
...
| Qwen 3 | 2025 | 0.6B–235B，含 Dense 与 MoE | thinking / non-thinking 模式、Dense/MoE 全尺寸矩阵 |
...
Qwen3 的重点不只是“模型更大”，而是把两种使用模式放进同一代模型体系：

| 模式 | 适合任务 | 工程影响 |
|---|---|---|
| Non-thinking | 闲聊、信息抽取、简单代码、低延迟客服 | 输出短、成本低、首 token 延迟更可控 |
| Thinking | 数学、复杂推理、规划、多步代码修复 | 输出长、token 成本高、需要更严格的停止和预算控制 |

这类“思考模式”说明一个重要事实：LLM 能力不只由预训练架构决定，还取决于 post-training、RL、推理时 token budget 和采样策略。
做产品选型时，不能只比较 benchmark 分数，还要比较同等成本下的正确率、响应时延和可控性。
...
def load_balancing_loss(gate_logits, n_experts):
    ...
    _, top1 = probs.max(dim=-1)

    # 每个 expert 实际收到 token 的比例
    tokens_per_expert = F.one_hot(top1, n_experts).float().mean(dim=0)
    # router 分配给每个 expert 的平均概率
    prob_per_expert = probs.mean(dim=0)

    return n_experts * torch.sum(tokens_per_expert * prob_per_expert)
```

真实 MoE 系统还会结合 capacity factor、token dropping、expert parallel、All-to-All 通信优化和 router z-loss。
辅助损失只解决“专家别被饿死”的一部分问题，不能替代高质量数据和稳定的分布式训练。
...
- 在多个公开 benchmark 上达到接近顶级闭源模型的水平

### 7.3 DeepSeek-R1：推理增强不只是架构问题

DeepSeek-R1 的关键价值在于展示了大规模 RL 可以显著增强推理能力。
它不是简单把 Transformer block 改得更复杂，而是在强基座模型上通过强化学习和冷启动数据，让模型学会生成更长、更结构化的推理轨迹。

| 阶段 | 作用 | 你应该关注什么 |
|---|---|---|
| Base model | 提供语言、知识、代码和基础推理能力 | 架构、数据、tokenizer、上下文 |
| Cold-start / SFT | 给 RL 一个可读、稳定的初始推理格式 | 数据质量、格式约束 |
| RL | 优化可验证任务上的推理策略 | reward 设计、长度膨胀、稳定性 |
| Distillation | 把大模型推理能力迁移到小模型 | 成本、泛化、蒸馏数据污染 |

工程启示：遇到“数学/代码/规划能力差”的问题时，架构不是唯一旋钮。
你还要看推理预算、训练后处理、验证器、数据蒸馏和是否允许长链路工具调用。
...
2024  LLaMA 3            128K 上下文，405B 参数
  ...
  │
2025  DeepSeek-R1        RL 推理增强
      Qwen 3             Dense/MoE + thinking/non-thinking 模式
```
...
| Qwen3-32B | 32B | 128K | GQA | 8 | 151K | SwiGLU | RMSNorm | RoPE | 未完全公开 |
| Qwen3-235B-A22B | 235B (22B↑) | 128K | GQA + MoE | 4 | 151K | SwiGLU | RMSNorm | RoPE | 未完全公开 |
...
# num_experts          → MoE expert 数（如果存在）
# num_experts_per_tok  → 每个 token 激活几个 expert（如果存在）
```
...
MoE 模型不能直接套 Dense 参数估算公式。你需要把 shared expert、routed expert、top-k 激活参数和 router 参数分开算：

```text
总参数 ≈ embedding + attention + shared_experts + routed_experts + lm_head
激活参数 ≈ embedding + attention + shared_experts + top_k * single_expert + lm_head
```

部署时还要注意：即使每个 token 只激活部分 expert，单机推理通常仍要能加载全部 expert 权重；
多机推理则会引入 expert parallel 和 All-to-All 通信成本。
...
### 14.6 忽视 post-training 和推理预算

同一个 base model，经过不同的 SFT、RLHF、DPO、RL 或蒸馏后，行为会完全不同。
推理模型还会通过更长的 reasoning tokens 换取正确率。
比较模型时要固定输出 token 预算、温度、工具调用权限和评测集，否则很容易把“花了更多 token”误认为“架构更强”。
