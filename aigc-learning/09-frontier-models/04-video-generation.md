...
> 本文按 `2026-06-30` 的公开资料组织；闭源视频系统只讨论公开能力和使用现象，不推断内部参数。
...
视频生成不是一个单一任务。不同任务对架构和工程约束完全不同。

| 任务 | 输入 | 输出 | 更关注 |
|---|---|---|---|
| T2V | 文本 | 视频 | 文本对齐、运动质量、多样性 |
| I2V | 图像 + 文本 | 视频 | 首帧保持、运动自然、主体一致 |
| V2V / 编辑 | 视频 + 指令 | 视频 | 时序一致、局部修改、不破坏背景 |
| Extend | 视频片段 | 后续视频 | 长时一致、记忆、镜头连续 |
| 数字人 | 音频/文本 + 人脸 | 视频 | 口型、身份、低延迟 |
| 世界模拟 | 状态/动作/文本 | 未来观测 | 物理一致、可控交互 |

如果只是产品宣传短视频，I2V + 模板化镜头常常比纯 T2V 更稳定。
如果要做可控编辑，视频扩散主干之外还需要 mask、光流、深度、tracking 或 reference attention。
...
### 3.1.1 3D VAE 是视频质量上限

很多人只盯着 DiT 参数量，但视频模型的第一道瓶颈常常是 3D VAE。

| 设计 | 影响 |
|---|---|
| 空间压缩率 | 压缩越强越省显存，但细节和小文字更差 |
| 时间压缩率 | 压缩越强越省计算，但运动细节和节奏更容易丢 |
| causal / non-causal | causal 更适合流式和扩展，non-causal 可利用未来帧重建 |
| tiling/slicing | 决定高分辨率推理是否 OOM |
| 重建损失 | 如果 VAE 重建就闪烁，扩散模型很难完全修复 |

调试视频生成质量时，先看 VAE reconstruction，再看去噪模型输出。
...
> Sora 没有公开完整架构和权重。可以学习其技术报告提出的 spacetime patch、可变分辨率/时长、world simulator 方向；
> 不应把社区推测的参数量、VAE 细节或训练数据规模当作事实。
...
> 视频模型更新很快。表格用于理解技术路线，不应用作实时排行榜。
> 选型时以官方模型卡、许可证、实际硬件测试和你自己的 prompt 集为准。

### 8.1 视频模型选型矩阵

| 约束 | 优先考虑 |
|---|---|
| 只有单张 24GB GPU | 低分辨率、短帧数、CPU offload、量化、小模型 |
| 需要中文 prompt | Wan、HunyuanVideo、CogVideoX 等中文数据更强的模型 |
| 需要可控主体 | I2V、参考图、多条件控制，而不是纯 T2V |
| 需要长视频 | 分段生成 + keyframe/overlap/consistency，比一次生成更现实 |
| 需要批量生产 | 推理速度、失败率、重试成本比单条 demo 质量更重要 |
| 需要商用 | 许可证、训练数据风险、水印和审核流程必须先确认 |
...
### 12.3 好的视频 caption 应该包含什么？

视频 caption 不能只描述静态画面。
...
| 镜头 | 低机位跟拍，轻微手持晃动 |
| 时间变化 | 狗先停下抖雪，然后继续奔跑 |
| 风格 | cinematic lighting, shallow depth of field |

训练数据如果缺少动作和镜头描述，模型就容易生成“高质量但不动”的视频。
这也是为什么商业视频模型的数据清洗和自动标注 pipeline 是核心壁垒。
...
```text
权重 + text encoder + 3D VAE + denoising activations + attention KV/中间张量
```
...
| 旋钮 | 影响 |
|---|---|
| 帧数 | 近似线性增加 token/latent 数 |
| 分辨率 | 近似按面积增加，720p 到 1080p 不是小升级 |
| patch size | patch 越小 token 越多，细节越好但 attention 更贵 |
| 去噪步数 | 近似线性影响耗时 |
| batch size | 视频生成通常 batch=1，批量生产靠队列而不是大 batch |
| offload/tiling | 省显存但显著增加延迟 |

开发流程建议：

1. 先用低分辨率、少帧数验证 prompt 和 pipeline。
2. 固定 seed 调通后再提高分辨率和帧数。
3. 每次只改一个旋钮，记录显存峰值和耗时。
4. 上线前用业务 prompt 集统计失败率和重试成本。
...
2025     Sora / Veo / Kling 等      商业系统继续迭代
```
...
架构学习要理解模型本体，但工程落地还要把后处理链路算进成本和质量。
...
| 对比 | 观察点 |
|---|---|
| 49 帧 vs 81 帧 | 运动连贯性、显存、耗时 |
| 480p vs 720p | 细节、失败率、OOM 风险 |
| T2V vs I2V | 主体一致性、可控性、多样性 |
